...
class TeslaLordBoss extends BossBase {
  ...
  void updateBoss(double dt) {
    ...
    final targetPos =
        playerPosition +
        Vector2(math.cos(orbitAngle) * 250, math.sin(orbitAngle) * 250);
    ...
    if (_towerPositions.length >= 2) {
      for (int i = 0; i < _towerPositions.length; i++) {
        ...
        final playerDist = _distToSegment(
  ...
  void renderBoss(Canvas canvas, Paint paint, double scale) {
    ...
    if (scale <= 1.01 && _towerPositions.length >= 2) {
      for (int i = 0; i < _towerPositions.length; i++) {
        ...
        // Glow esterno (alone)
        _towerGlowPaint.color = neonColor.withValues(alpha: 0.25 * towerPulse);
        canvas.drawCircle(Offset(tCx, tCy), 18, _towerGlowPaint);
        // Core bianco pulsante
        _towerCorePaint.color = const Color(
          0xFFFFFFFF,
        ...
        _drawLightning(canvas, tCx, tCy, cx + nextOffset.x, cy + nextOffset.y);
    ...
    if (scale <= 1.01) {
      ...
      _arcPaint.color = neonColor.withValues(alpha: 0.2 + fieldPulse * 0.2);
      _arcPaint.strokeWidth = 1.5;
      canvas.drawCircle(Offset(cx, cy), r * 1.3, _arcPaint);
      canvas.save();
      canvas.translate(cx, cy);
      canvas.rotate(-_sparkPhase * 0.4);
      _arcPaint.color = const Color(
        0xFFFFFFFF,
      ).withValues(alpha: 0.15 + fieldPulse * 0.15);
...
class _TeslaBullet extends PositionComponent
