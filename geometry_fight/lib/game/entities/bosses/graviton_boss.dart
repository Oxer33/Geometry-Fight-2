  ...
  void renderBoss(Canvas canvas, Paint paint, double scale) {
    ...
    for (int ring = 0; ring < 3; ring++) {
      ...
      _ringPaint.color = const Color(
        0xFFFFD700,
      ).withValues(alpha: 0.3 - ring * 0.08);
    ...
    if (scale <= 1.01) {
      for (int i = 0; i < 4; i++) {
        ...
        _lensingPaint.color = const Color(0xFFFFD700).withValues(
          alpha: (0.2 - i * 0.04) * (0.7 + math.sin(_gravPhase + i) * 0.3),
        );
    ...
    if (scale <= 1.01) {
      ...
      for (int i = 0; i < 8; i++) {
        ...
        _filamentPaint.color = const Color(
          0xFFFFD700,
        ).withValues(alpha: fPulse);
        canvas.drawLine(
          Offset(
            cx + math.cos(fAngle) * r * 0.42,
            cy + math.sin(fAngle) * r * 0.42,
          ),
          Offset(cx + math.cos(fAngle) * fLen, cy + math.sin(fAngle) * fLen),
          _filamentPaint,
        );
      }
    }
    ...
    if (scale <= 1.01) {
      final pulse = (_isPulling ? 0.7 : 0.4) + math.sin(_gravPhase * 4) * 0.2;
...
class _GravBullet extends PositionComponent
  ...
  Future<void> onLoad() async {
    _velocity = direction.normalized() * 180;
  }
